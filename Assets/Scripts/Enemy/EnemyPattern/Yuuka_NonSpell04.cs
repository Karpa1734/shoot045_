...
public class Yuuka_NonSpell04 : BossPatternBase
{
    ...
    IEnumerator BitAttackRoutine()
    {
        ...
        while (true)
        {
            ...
            while (speed < 3.5f) {
                ...
                CreatePolygonShot(BLUE[0], transform.position, edges, bulletCount, speed, angle, 5);
            ...
            while (speed < 3.5f)
            {
                ...
                SEManager.Instance.Play(SEPath.SHOT1, 0.5f);
                // transform.position を渡すことで、移動中の現在地から発射される
                CreatePolygonShot(WHITE[0], transform.position, edges, bulletCount, speed, angle, 5);
